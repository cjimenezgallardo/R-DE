...
vbf.test(datos$y~as.factor(datos$x),data=datos) # 



# Anova Completamente Aleatorizado o a Una Via (oneWay)

Anova<-aov(datos$y~as.factor(datos$x)) # si la VI es numerica utilice as.factor, si no puede crear una variable factor
summary(Anova) 
...
summary(ResGames)
plot(ResGames,main="Test Games-Howell")

#-----------------------------------------------------------------------------

ResT2<- tamhaneT2Test(Anova)
summary(ResT2)
plot(ResT2)
#-----------------------------------------------------------------------------

ResT3<-tamhaneDunnettTest(Anova)# utiliza el primero como control
summary(ResT3)
plot(ResT3)
#-----------------------------------------------------------------------------
